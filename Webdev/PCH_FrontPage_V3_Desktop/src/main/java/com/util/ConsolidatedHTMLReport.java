...
public class ConsolidatedHTMLReport extends BaseClass {

	private FileWriter cons_file_out;
	private PrintWriter cons_print_out;
	private final String testReportPath = "./CustomHTMLReport/";
	private final String testScreenshotPath = System.getProperty("user.dir") + "//screenshots//";
	private final String testLogPath = System.getProperty("user.home") + "//Logs//";
	...
	/***
	 * 
	 * To depict the suite overall execution details - Passed, Failed and Skipped
	 * with Percentage Provide Pie-Chart based on the suite execution results
	 * 
	 * @param timeDuration
	 * @param count
	 * 
	...
	protected void suiteOverAllExecutionDetails(String timeDuration, int[] count) {
		...
		cons_print_out.println(
				"<tr><td class=\"left-active-color\">Application Environment</td><td class=\"right-active-color\">"
						+ xmlReader(ENVIRONMENT, "BaseURL").replaceAll("(https*://)(\\w+-*\\w+)(.*)", "$2").trim()
								.toUpperCase()
						+ "</td></tr>");
	 ...
	 * 
	 * @param PrintWtriter object of the HTML file
	 */
	...
	/**
	 * Create a Logs folder to send reports in Email
	 * 
	 * @author mperumal
	 */
	protected void LogsToSendInEmail() {
		System.out.println("Inside Logs copy to send report in Email");
		File file = new File(testReportPath + "HTMLReports//Logs");
		if (!file.exists()) {
			file.mkdir();
			try {
				copyFolder(new File(testReportPath + currentTime + "//Logs//"), file,
						testScreenshotPath + currentTime + "//Logs//");
			} catch (IOException e) {
				reportLog.error("Error in Logs copy to send report in Email" + e.getMessage());
			}
		} else {
			reportLog.info("Logs folder already exist");
			try {
				FileUtils.deleteDirectory(file);
			} catch (IOException e) {
				reportLog.error("Error in delete HTMLReports/Logs folder " + e.getMessage());
			}
			LogsToSendInEmail();
		}
	}

...
public class TabAcquisitionQuizPage extends PageObject {
	...
	public TabAcquisitionQuizPage(WebDriver driver) {
		super(driver);
	}
	...
	public void gotoQuizzesAcqPath() throws URISyntaxException, InterruptedException, IOException {
		...
		executor.executeScript("arguments[0].click()", getDriver().findElement(By.cssSelector("article.qdetail-item >a")));
		...
		if (promotionFromArticle.trim().equalsIgnoreCase(qapromotion.trim())||promotionFromArticle.trim().equalsIgnoreCase(stgpromotion.trim())) {
			acqUrl = url + "/" + "?" + "acq=" + promotionFromArticle.trim();
	...
	public void fullPathRegistrationTokensVerification(String path) throws InterruptedException, IOException {
		String verb;
		String verbTokens;WebElement tokenHistory;JavascriptExecutor executor;
		switch (path) {
			case fullpath:
				tokenHistory=element(tokenHistoryTitle);
				WaitHelper.forceWait(5000);
				((JavascriptExecutor) getDriver()).executeScript("arguments[0].scrollIntoView();", tokenHistory);
				executor = (JavascriptExecutor) getDriver();
				executor.executeScript("arguments[0].click()",tokenHistory);
				waitForRenderedElementsToBePresent(quizcompAndRegVerbiage);
	...
	public boolean landOnRegPageAfterPathCompletion() throws InterruptedException {
		getDriver().manage().timeouts().pageLoadTimeout(10, TimeUnit.SECONDS);
		WebElement claimNow=element(ClaimNowBtn);
		WaitHelper.forceWait(5000);
		jseClick(claimNow);
		waitForRenderedElementsToBePresent(regFormHeaderTitle);
		return element(regFormHeaderTitle).isDisplayed();
	}
	...
	public void halfWayPath() throws InterruptedException, IOException {
		List<WebElementFacade> quizAnswers = findAll(answers);
		for (int i = 0; i < quizAnswers.size(); i = i + 2) {
			WebElement answersPanel= element(answers).findElements(answers).get(i);
			((JavascriptExecutor) getDriver()).executeScript("arguments[0].scrollIntoView();", answersPanel);
			System.out.println("Quiz--->" + "i" + i + element(answers).findElements(answers).get(i).getText());
			((JavascriptExecutor) getDriver()).executeScript("window.scrollBy(0,-400)");
			waitABit(2000);

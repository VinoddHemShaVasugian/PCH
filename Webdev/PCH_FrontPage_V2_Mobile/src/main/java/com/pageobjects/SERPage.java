...
public class SERPage extends BaseClass {
	...
	public boolean verifyLeftsidebar() {
		System.out.println(DriverManager.get_event_firing_webdriver().findElements(webResults).size());
		return DriverManager.get_event_firing_webdriver().findElements(webResults).size() > 1;
	}
	...
	public boolean verifiy_sortingShoppingResults(String searchterm) {
		...
		status = DriverManager.get_event_firing_webdriver().getCurrentUrl()
		...
		status = status && DriverManager.get_event_firing_webdriver().getCurrentUrl()
	 ...
	 */
	public void verify_search_results_page(String searchterm) {
		assertEquals(DriverManager.get_event_firing_webdriver().getCurrentUrl(),
				xmlReader(ENVIRONMENT, "BaseURL") + "search?q=" + searchterm);
	}
	...
	/**
	 * To verify the App Search Results page url.
	 */
	public boolean verify_app_search_results_url(String searchterm) {
		return (DriverManager.get_event_firing_webdriver().getCurrentUrl()
				.equals(xmlReader(ENVIRONMENT, "BaseURL") + "appsearch?q=" + searchterm));
	}

...
public class TokensForWatchingVideoTests extends BaseTest {
	...
	public void tokensForWatchVideoConfiguration() {
		...
		Assert.assertEquals(joomlaPage.setTextProperty("Token Amount", 50),
				"success");
	}
	...
	public void testTokensForWatchingVideos() throws InterruptedException {
		...
		Assert.assertTrue(aboutPage.isVideoExists("videoClip"),
				"Video does not display");
		Common.sleepFor(3000);
		// do not watch complete video
		homePage.goToAboutTokens();
		Assert.assertTrue(aboutPage.isVideoExists("tokenVideo"),
				"Video does not display");
		Common.sleepFor(3000);
		...
		Assert.assertFalse(tokenHistoryPage.getActivityName().contains(
		...
		Assert.assertTrue(tokenHistoryPage.getActivityName().contains(
				"For Watching the Video!"));
		Assert.assertTrue(tokenHistoryPage
				.getEventCount("For Watching the Video!") == 2);
		Assert.assertEquals(
				tokenHistoryPage.getTokenForEvent("For Watching the Video!"),
				String.valueOf(50));
	...
	public void testTokensForWatchingVideosNextDay()
		...
		myAccountPage.navigateToTab(MyAccountTab.Token_History);
		Assert.assertTrue(tokenHistoryPage
				.getEventCount("For Watching the Video!") < 3);

		// update database
		DBUtils.updateTokensForWatchingVideos(randomUser_1.getEmail(),
				"token_awarded_watchingvideo_about");
		DBUtils.updateTokensForWatchingVideos(randomUser_1.getEmail(),
				"token_awarded_watchingvideo_tokens");
		...
		Assert.assertTrue(tokenHistoryPage.getActivityName().contains(
				"For Watching the Video!"));
		Assert.assertTrue(tokenHistoryPage
				.getEventCount("For Watching the Video!") == 4);
		Assert.assertEquals(
				tokenHistoryPage.getTokenForEvent("For Watching the Video!"),
				String.valueOf(50));

...
public class GuidedSearchSearchBoxTests extends BaseTest {
	...
	@Test(groups={GroupNames.Desktop}, description="Validate search box is displayed if enabled - TestID-29928")
	public void gsSearchBox_Enable(){
		
		// load GS for unrecognized users and validate 
		homePage.load();
	...
	@Test(groups={GroupNames.Desktop}, description="Validate search box is not displayed if disabled - TestID-29928")
	public void gsSearchBox_Disable(){
		
		// load GS for unrecognized users and validate 
		homePage.loadGuidedSearch("	SWGSVamsiL2Disabled");
		gs.validateGs("L2-D-T-6E-135");
	...
	@Test(groups={GroupNames.Desktop}, description="Validate if we are able to search using top and bottom search boxes - TestID-29928")
	...
	@Test(groups={GroupNames.Desktop}, description="Top Search box - Validate if predictive search terms are shown as per the keyword provided - TestID -29929")
	...
	/*We don't have bottom search box in Desktop GS*/	
	...
	/*@Test(groups={GroupNames.Desktop}, description="Top Search box - Validate if predictive search terms are not shown as per the keyword provided - TestID-29929")
	public void testPredictiveSearchSuggestions_TopSearchBox_OFF(){
			
		// Go to GS where predictive text functionality is disabled
		homePage.loadGuidedSearch("SWGSL2PredictivetextOff1");
		gs.validateGs("L2-D-T-6E-136");
		
		// Validate predictive text functionality
		homePage.loadGuidedSearch("SWGSL2PredictivetextOff1");
		List<String> suggestions = header.getPredictiveSearchSuggestions(searchKeyword);
		Assert.assertNotNull(suggestions,"Suggestions are still displayed - "+suggestions);

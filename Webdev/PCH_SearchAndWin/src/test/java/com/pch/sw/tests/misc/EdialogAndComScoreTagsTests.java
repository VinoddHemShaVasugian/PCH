...
public class EdialogAndComScoreTagsTests extends BaseTest {
	...
	@Test(description = "[1]Evergage_tags - 19089")
	...
	@Test(description = "[1]UnRecohnizedUser-Evergage support with GMT - 22649")
	public void evergageUnrecognizedUser() {

		List<String> actualPageSource = new ArrayList<String>();
		// String envi = "PCH.env = \"qa\"";
	...
	@Test(description = "[1]RecognizedUser-Evergage support with GMT - 22648")
	public void evergageRecognizedUser() {
		...
		actualPageSource.add("PCH.env = \"" + Environment.getEnvironment().toLowerCase() + "\";");
		...
		for (int i = 0; i < actualPageSource.size(); i++) {
			if (pageSource.contains(actualPageSource.get(i))) {
				Assert.assertTrue(pageSource.contains(actualPageSource.get(i)));
			}
		}
	}

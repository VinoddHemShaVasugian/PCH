...
public class CustomListener extends TestListenerAdapter implements IReporter  {
	...
	public void generateReport(List<XmlSuite> arg0, List<ISuite> arg1,String arg2) {	
		...
		try{
			String PCHUSERcontent = "";
			if(!(System.getProperty("comments")==null)){
				PCHUSERcontent = System.getProperty("comments");
			}
			...
			for(TestNG_Suite testsuite:suits){
				System.out.println("*** Suite Name *** "+testsuite.suiteName);
				for(TestNG_Test te_st:testsuite.testNG_tests_inSuite){
					...
					eu.insertLogo();
					eu.writeText("Result of "+te_st.testName,col+1, row, 20, WritableFont.TIMES, true, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.LIGHT_BLUE, Colour.WHITE);	
					eu.mergeCell(col+1, row, col+13, row);
					row++;
					for(TestNG_Class test_klass:te_st.testNG_classes_inTest){
						...
						eu.mergeCell(col, row, col+4, row);
						row++;
						
						System.out.println("*** Class Name ****** "+test_klass.clasName);
						for(TestNG_MethodRunMetadata test_method:test_klass.testNG_methods_inClass){
							...
							//eu.writeText(PCHUSERcontent, col, row++, 9, WritableFont.TIMES, false, Alignment.LEFT, VerticalAlignment.CENTRE, Colour.BLACK, Colour.WHITE);
							eu.addComment(test_method.description,col++,row);
							if(test_method.executionResult==ITestResult.FAILURE){
								++counter.failed;
								eu.writeText("", col++, row, 9, WritableFont.TIMES, false, Alignment.LEFT, VerticalAlignment.CENTRE, Colour.BLACK, Colour.WHITE);
							...
							System.out.println("*** Method Name ******** "+test_method.thisMethodname);
							row++;
						}						
					}
				...
				row=3;
				col=5;
				eu.createSheet("GS-S&W Results", 0);
				eu.insertLogo();
				...
				row=row+2;				
				col=col+2;
				eu.writeText("Test Name", col-2, row, 11, WritableFont.TIMES, false, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.BLACK, Colour.GREY_25_PERCENT);
				eu.writeText("Passed", col++, row, 11, WritableFont.TIMES, false, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.BLACK, Colour.GREY_25_PERCENT);
				eu.writeText("Failed", col++, row, 11, WritableFont.TIMES, false, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.BLACK, Colour.GREY_25_PERCENT);
				eu.writeText("Skipped", col++, row, 11, WritableFont.TIMES, false, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.BLACK, Colour.GREY_25_PERCENT);
				eu.writeText("Pass Rate", col++, row, 11, WritableFont.TIMES, false, Alignment.CENTRE, VerticalAlignment.CENTRE, Colour.BLACK, Colour.GREY_25_PERCENT);
				int passed =0;
				int failed=0;
				int skipped=0;

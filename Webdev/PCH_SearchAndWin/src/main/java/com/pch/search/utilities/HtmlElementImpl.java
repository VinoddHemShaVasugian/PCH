...
public class HtmlElementImpl implements HtmlElement {
	...
	public List<HtmlElement> findElements(final By by) {
		...
		/*
		 * for(WebElement e: element.findElements(by)){ elements.add(new
		 * HtmlElementImpl(e,driver)); }
		 */
	...
	public void scrollDownAndClick() {
		...
		element.click();
		/*
		 * while (true) { try { element.click(); return; } catch
		 * (WebDriverException wde) { String exceptionMesssage =
		 * wde.getMessage(); if (exceptionMesssage.contains(
		 * "Element is not clickable at point") || exceptionMesssage.contains(
		 * "element not visible")) { ((JavascriptExecutor)
		 * driver).executeScript("window.scrollBy(0,50)", new Object[0]);
		 * element.click(); // driver.executeScript("window.scrollBy(0,50)", new
		 * // Object[0]); } else { CustomLogger.logException(wde);
		 * wde.printStackTrace(); return; } } }
		 */
	}
	...
	public void scrollUpAndClick() {
		...
		/*
		 * while (true) { try { element.click(); return; } catch
		 * (WebDriverException wde) { if (wde.getMessage().contains(
		 * "Element is not clickable at point")) { ((JavascriptExecutor)
		 * driver).executeScript("window.scrollBy(0,-50)", new Object[0]); //
		 * driver.executeScript("window.scrollBy(0,-50)", new // Object[0]); }
		 * else { CustomLogger.logException(wde); wde.printStackTrace(); return;
		 * } } }
		 */
	}
	...
	public void waitForVisible() {
		...
		/*
		 * ExpectedCondition<Boolean> customCondition = new
	...
	public void removeFromDOM() {
		((JavascriptExecutor) driver).executeScript("arguments[0].parentNode.removeChild(arguments[0])",
	...
	public void forceClick() {
		if (!Environment.getBrowserType().equals("ie")) {
			...
			JavascriptExecutor jse = (JavascriptExecutor) driver;
			jse.executeScript(
					"var evt = document.createEvent(\"MouseEvents\");evt.initMouseEvent('click', true, true,arguments[0].ownerDocument.defaultView, 1, 0, 0, 0, 0, false,false, false, false, 1, null);arguments[0].dispatchEvent(evt)",
					this.getWrappedElement());
		}
	}
	...
	public List<HtmlElement> getOptions() {
		 ...
		 * listItems.addAll((Collection<? extends HtmlElement>) new
		 * Select(element).getOptions()); return listItems;
		 */
		return null;

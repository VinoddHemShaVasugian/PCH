...
public class MyAccountPage extends Action {
	...
	public void navigateToTab(MyAccountTab tab){
		...
		if(device.equalsIgnoreCase("mobile")){	
			...
			CustomLogger.log("Navigating to :"+tab);
			driver.waitForBrowserToLoadCompletely();
			waitForMyAccountPageToLoad();
			String tabName = tab.getTabName();
			HtmlElement tabElement= getTabElement(tabName);
			HtmlElement parent_tabElement = tabElement.findElement(By.xpath(".."));
			if(parent_tabElement.getAttribute("class").contains("active")){
				CustomLogger.log(tabName+" already selected.");
				return;
			}else{
				parent_tabElement.click();
		//		driver.waitForBrowserToLoadCompletely();
			}
		}

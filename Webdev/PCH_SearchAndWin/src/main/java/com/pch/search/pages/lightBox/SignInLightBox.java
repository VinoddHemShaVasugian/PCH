...
public class SignInLightBox extends LightBox {
	...
	protected HtmlElement locateLightBox() {
		HtmlElement lghtBox = driver.findElement(By.id("pch-login-form"));
	...
	public HtmlElement forgotPasswordLink() {
		HtmlElement element = locateLightBox();
		return element.findElement(By.linkText("Click Here"));
	}
	...
	public int signIn(User user) {
		...
		try {
			...
			/*
			 * If we Login with Valid User, then it was looking for unsuccessful
			 * login error message, which will never come with valid user.. so
			 * implemented a if stmnt, if title doesn't matches to Frontpage
			 */
			...
			try {
				if (!titleOfPage.contains("GuidedSearch")) {
					...
					if (device.equalsIgnoreCase("mobile")) {
						...
						pleaseCorrect = driver.findElement(
								By.xpath("//div[@class='sso-outer-shell']/descendant::ul[@class='error']/li"), 14);
					}
				...
				CustomLogger.log(e.getMessage().toString());
				LightBox lb = new OptinLightBox(driver);
				lb.dismissLightBox();
				return 1;
			}

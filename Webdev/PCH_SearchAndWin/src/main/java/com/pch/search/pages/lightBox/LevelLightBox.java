...
public class LevelLightBox extends LightBox {
	...
	/*private HtmlElement levelUpCelebrationFrameHolder(){
		return driver.findElement(By.id("levelUpCelebrationIframeHolder"));
	}*/
	...
	protected HtmlElement locateLightBox() {
		try{
			...
			preCelebrationLoader.waitTillNotVisible(3);
			...
			if(driver.getCountOfElementsWithXPath(levelImageXPath)>0){
				...
				lastDisplayedUserLevel=imgSrcAttribute.substring(imgSrcAttribute.lastIndexOf("-")+1, imgSrcAttribute.indexOf(".png"));
	...
	public void dismissLightBox() {
		...
		do{
			...
			if(lightBox!=null){
				...
				/*If Dismiss Button is not leaving, it means Special reward message is being displayed.
				 So closing it first.
				  */
				try{
			...
			driver.switchTo().defaultContent();
			isCelebrationFrameHolderPresent = driver.getCountOfElementsWithXPath("//div[@id='levelUpCelebrationIframeHolder']")!=0;
		...
		do{
			...
			if(lightBox!=null){
				...
				//}catch(TimeoutException toe){				
					 ...
					 */
					CustomLogger.log("Light box didn't leave, checking special reward message");
					HtmlElement specialRewardElement = driver.findElement(By.xpath("//div[@id='BAMContainer']/descendant::div[@class='level_msg']"));					
					specialRewardElement.click();
					CustomLogger.log("Closed special reward message");
					dismissBtn.waitTillNotPresent();
				//}				

...
public class PageFactory  {
	...
	private static Map<String,BrowserDriverImpl> browserNDriverMap = null;

	protected static <T> T getPageInstance(Class<T> klass) throws Exception{
		T pageObjectInstance = null;
		try {
			if(getBrowserNDriverMap()==null)
				setBrowserNDriverMap(new ConcurrentHashMap<String, BrowserDriverImpl>());
			...
			for(Field field:fields){
				if(field.getType().equals(BrowserDriver.class)){
					...
					if(!getBrowserNDriverMap().containsKey(browserKey)){
						...
						getBrowserNDriverMap().put(browserKey, Browser.getBrowserInstance());						
					}
					field.set(pageObjectInstance,getBrowserNDriverMap().get(browserKey));	
	...
	public static Map<String,BrowserDriverImpl> getBrowserNDriverMap() {
		return browserNDriverMap;
	}

	public static void setBrowserNDriverMap(Map<String,BrowserDriverImpl> browserNDriverMap) {
		PageFactory.browserNDriverMap = browserNDriverMap;
	}

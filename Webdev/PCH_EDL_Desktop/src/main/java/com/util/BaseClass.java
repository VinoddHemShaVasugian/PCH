...
public class BaseClass extends WrapperFunctions {
	...
	public void setup(ITestContext ctx) throws Exception {
		...
		String browser_name = ctx.getCurrentXmlTest().getAllParameters().get("browser_name");
		String grid_url = ctx.getCurrentXmlTest().getAllParameters().get("grid_url");
		if (browser_name != null && grid_url != null && !browser_name.isEmpty() && !grid_url.isEmpty()) {
			createInstance(browser_name, grid_url);
		} else {
			createInstance();
		}

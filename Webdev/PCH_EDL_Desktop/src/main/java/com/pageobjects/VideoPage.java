...
public class VideoPage extends BaseClass {
	...
	private static final VideoPage instance = new VideoPage();
	...
	private final By video_title = By.id("video-player-title");
	...
	private final By selected_video_title_on_right_rail_playlist = By
	...
	private final By edl_video_page_title = By.xpath("//div[@class='row section-header-row'][1]//a[@class='section-header__link']");	
	private final By overlay_title_in_video_player = By.xpath("//aside[@class='fp-video-overlay fp-video-overlay-video-title']");
	private final By next_video_to_be_played = By.className("fp-video-overlay-next-video__title");
	private final By right_rail_video_playlist = By.xpath("//ul[@class='fp-video-playlist']/li");
	...
	/**
	 * Navigate to the respective sub-category page by using the given param
	 * label name.
	 * 
	 ...
	 * @return
	 */
	public String get_video_title_on_right_rail_playlist() {
		return getText(selected_video_title_on_right_rail_playlist, 30);
	}
	...
	/** Return the EdL Video Page title which is shown in line with date on the left side */
	public String get_video_page_edl_title(){
		return getText(edl_video_page_title, 5);
	}
	
	/** Once the video loads with the video its title is shown on the top */
	public boolean verify_video_overlay_title(){
		
		return elementPresent(overlay_title_in_video_player);
	}	
	...
	/** After video completion the text of the next video to be played is displayed above play button. This method
	 * returns that value 
	 * @return
	 */
	public String get_next_video_to_be_played_text(){
		moveToElement(video_player);
		return getText(next_video_to_be_played,5000);
	}
	...
	public boolean verify_token_already_claim_button() {
		return elementVisibility(tokens_already_claimed_button,5000);
	}
	
	public void click_random_video_on_right_playlist(){
		
		int video_count = get_webelements_list(right_rail_video_playlist).size();
		button(get_webelements_list(right_rail_video_playlist).get(rand(1, video_count)),20);

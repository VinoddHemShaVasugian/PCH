...
public class DB_Connector extends WrapperFunctions {
	...
	public int updateMYSQLQuery(String Query) throws SQLException {
		...
		try {
			...
			log.info(e.getMessage());
			System.out.println("Exception noticed on running following query: " + Query);
		} finally {
			log.info("Closing the DB connection");
			close_mysql_connection();
		}
...
//		 long expire_timetamp = modifyCurrentDateByOffset("year", -1);
	...
	public void updateTokenAwardLinkPromo(String userEmail) throws SQLException {
		...
		String user_id = getUserIdFromEmail(userEmail);
		Long timeStamp = new java.util.Date().getTime() / 1000L;
//		Long timeStamp = 1514764800L;
//		String query = "update sso_user_data set value = 0 , expires = " + timeStamp + " where user = '" + user_id
//				+ "' and item like 'tokenawardedlinkpromotion1000';";
		String query = "update sso_user_data set value = 0 , expires = " + timeStamp + " where user = '" + user_id
				+ "' and item like 'token_awarded_linkpromotion_196';";// Added by Vinoth for SegIdTokens
	...
	/**
	 * Assign segment to user
	 * 
	 * @param userEmail & segment
	 * @author vsankar
	 * @throws SQLException
	 */
	public void updateSegment(String userEmail, String segment) throws SQLException {
		log.info("Updating DB: Last Login timestamp for: " + userEmail);
		// To get the User Id of the user via OAT and Email by User API.
		String user_id = getUserIdFromEmail(userEmail);
		System.out.println("user_id: "+user_id+" & userEmail: "+userEmail);
		Long timeStamp = new java.util.Date().getTime() / 1000L;
		String query = "update sso_user_data set value = '[\"" + segment + "\"]' , expires = " + timeStamp + " where user = '"
				+ user_id + "' and item like 'Segments';";
	...
	public LinkedHashMap<String, String> get_video_log_details(String user_email, String video_title) {
		...
		String query = "select video_title, category, tokens, claimed, device, gmt, oat from video_log where user_id = "
				+ user_id + " and video_title = '" + video_title + "' order by `TIMESTAMP` desc limit 1;";
	...
	public String get_healthination_details(String user_email) {
		...
		// To get the User Id of the user via OAT and Email by User API.
		String user_id = getUserIdFromEmail(user_email);
		String query = "select value from sso_user_data where user = " + user_id
				+ " and item = 'desktopHealthiNationTokens';";
	...
	public String get_daily_progress_mission_details(String user_email) {
		...
		// To get the User Id of the user via OAT and Email by User API.
		String user_id = getUserIdFromEmail(user_email);
		String query = "select * from sso_user_data where user = '" + user_id
				+ "' and item like 'desktopdailygameplay'";
		return executeQueryAndVerifyValue(query, "value");
	}
	...
	public LinkedHashMap<String, String> get_mulitple_columns_values_of_user(String query) {
		...
		try {
			...
			log.error("Error in retreiving the mulitple column vlaues for the user:" + SQL.getSQLState());
	...
	/**
	 * Retrieve the User Id of the userby using Email via Central Service and User
	 * API's.
	 * 
	 * @return User Id
	 * @author mperumal
...
//					+ getUserGMTOATFromEmail(user_email, "OAT");
			to_get_user_id_from_user_api = "http://userapi.stg.pch.com/api/user/"
					+ getUserGMTOATFromEmail(user_email, "OAT");
			user_id = WrapperFunctions.getResponseParamFromGETCall(to_get_user_id_from_user_api, "id", "data");
		} else {
			// To get the User Id of the user via OAT and Email by User API's.
			to_get_user_id_from_user_api = "http://userapi.qa.pch.com/api/user/"+ getUserGMTOATFromEmail(user_email, "OAT");
			user_id = WrapperFunctions.getResponseParamFromGETCall(to_get_user_id_from_user_api, "id", "data");
		}
		return user_id;
	}
	 ...
	 * @author vsankar
	 */
	public String get_sweep_return_details(String user_email) {
		log.info("Retrieving sweep crmn details for user: " + user_email);
		System.out.println("Retrieving sweep crmn details for user: " + user_email);
		String user_id = getUserIdFromEmail(user_email);
		String query = "select value from sso_user_data where user = " + user_id
	...
	/**
	 * Delete sweeps property for an user
	 * 
	 * @param user_email
	 * @author vsankar
	 */
	public void delete_CRMN_value(String userEmail) throws SQLException {

		log.info("Updating DB: updating Registration Date for: " + userEmail);
		String user_id = getUserIdFromEmail(userEmail);
		String query = "DELETE FROM sso_user_data WHERE user="+user_id+" AND item='desktopSweepReturnValue'";
		updateMYSQLQuery(query);
		log.info(String.format("Sweeps property desktopSweepReturnValue is deleted for user: ", userEmail));
	}
}

...
public class SerpSteps extends ScenarioSteps {
	...
	public LinkedHashMap<String, String> getSearchDetails(String gmt) throws SQLException {
		String query = "select SearchType,UID,Edid,Current_Url,UserSignedIn, Nfsp,BlingoID,DeviceType,TSRC,TSRC2,SegID,Segment,cbl,FrontPageFlag, GS_Flag, GS_Category, GS_Position from search_activity_log where UID='"
				+ gmt + "'";
		return dbHelper.getMulitpleColumnValues(query);
	}

	@Step
	public String[] clickAdMarketPlaceTile() {
		return serpPage.clickAndReturnAdMarktePlaceTileName();
	}

	@Step
	public void clickAdMarketPlaceTileByPosition(int pos) {
		serpPage.clickAdMarktePlaceTileNameBasedOnGivenPos(pos);
	}

	@Step
	public boolean verifyAdTilesOnPage(String adTileName) {
		getDriver().navigate().refresh();
		return serpPage.verifyAdTileOnPage(adTileName);
	}
	...
	@Step
	public String getAdTrackCountProp(String propName) throws IOException, NumberFormatException, SQLException {
		String userId = WebServiceClient.getInstance().getUserIdFromEmail(RegistrationPage.regGenerator.getEmail());
		String query = "select value from sso_user_data where user='" + userId + "' and item = '" + propName + "';";

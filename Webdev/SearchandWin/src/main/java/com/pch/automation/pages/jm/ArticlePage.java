...
/**
 * @author mperumal Contains page functions for the SW Joomla login page
 * 
 */
public class ArticlePage extends PageObject {
	...
	public void searchForArticle(String articleName) {
		...
		for (WebElementFacade ele : ele_list) {
			if (ele.getText().equalsIgnoreCase(articleName)) {
				// evaluateJavascript("arguments[0].click();", ele);
				try {
				clickOn(ele.waitUntilClickable());
				}catch (Exception e) {
					waitABit(8000);
					clickOn(ele.waitUntilClickable());
				}				
	 ...
	 * 
	 * @param labelName
	 *            - Value need to be retrieve. Ex: Description
	...
	public String getValueByLabelName(String... labelName) {
		...
		String value = element(
				labelName.length > 1 ? elementList.get(Integer.parseInt(labelName[1]) - 1) : elementList.get(0))
						.getTextValue().trim();
		if (value != null && value.isEmpty() && value.equals("")) {
			value = (String) evaluateJavascript("return arguments[0].getAttribute('value');", element(
					labelName.length > 1 ? elementList.get(Integer.parseInt(labelName[1]) - 1) : elementList.get(0)));
		}
		return value;
	}
	 ...
	 * 
	 * @param labelName
	 *            - Field need to be modify/update. Ex: Description
	 * @param value
	 *            - Replacing text
	 * 
	 * @author mperumal
	 ...
	 * 
	 * @param keyName
	 *            - Value need to be retrieve. Ex: samsBanner
	...
	public String getValueByKeyName(String keyName) {
		...
		String value = element(by).getTextValue().trim();
		if (value != null && value.isEmpty() && value.equals("")) {
			value = (String) evaluateJavascript("return arguments[0].getAttribute('value');", element(by));
		}
		return value;
	}
	 ...
	 * 
	 * @param keyName
	 *            - Field need to be modify/update. Ex: samsBanner
	 * @param value
	 *            - Replacing text
	 * 
	 * @author mperumal
	 ...
	 * 
	 * @param labelName
	 *            - Value need to be retrieve. Ex: Segments Included
	 ...
	 * 
	 * @param labelName
	 *            - Drop down need to be select. Ex: Segments Included
	 * @param value
	 *            - Value to be select
	 * @author mperumal
	 * 

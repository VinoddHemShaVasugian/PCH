...
public class FBUserTests extends BaseClass {
	...
	@Test(priority = 3, groups = { "DESKTOP",
			"TABLET" }, description = "Verify the PCH Properties on Uni Nav bar for Social user", testName = "RT-04214:Social user test scenario [D/T]")
	public void verify_uni_nav_bar() throws Exception {
		test_Method_details(3, "Verify the PCH Properties on Uni Nav bar for Social user");
		// Step 1
		test_step_details(1, "Create new FB user and go to FP");
		invokeBrowser(xmlReader(ENVIRONMENT, "CentralServicesPageURL"));
		invokeBrowser(centralpage.createSocialUser()[1]);
		lb_instance.close_welcome_optin_lb();
		assertTrue(home_instance.verify_complete_registration());
		step_validator(1, true);

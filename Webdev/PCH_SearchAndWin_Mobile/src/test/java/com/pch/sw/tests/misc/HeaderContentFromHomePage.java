...
public class HeaderContentFromHomePage extends BaseTest {
	...
	@Test(groups = { GroupNames.Mobile }, description = "[1]RulesAndFacts_Mobile [M]16913")
	public void rulesAndFactsGuestUser() {
		...
		if (expectedOfficialRules.size() == actulaSearchOfficialRules.size()) {
			for (int i = 0; i < expectedOfficialRules.size(); i++) {
				CustomLogger.log(
						"Expected : " + expectedOfficialRules.get(i) + " Actual : " + actulaSearchOfficialRules.get(i));
				Assert.assertTrue(expectedOfficialRules.get(i).contains(actulaSearchOfficialRules.get(i)));
				// CustomLogger.log(expectedOfficialRules.get(i)+" :
				// "+actulaSearchOfficialRules.get(i));
			}
		}
	...
	@Test(groups = { GroupNames.Mobile }, description = "[1]SweepstakesFactsContentHomePage - 16556")
		...
		for (int cycle = 0; cycle < 2; cycle++) {
			...
			if (expectedStakesContent.size() == actualStakesContent.size()) {
				for (int i = 0; i < expectedStakesContent.size(); i++) {
					CustomLogger.log(
							"Actual : " + actualStakesContent.get(i) + " Expected : " + expectedStakesContent.get(i));
	...
	public void sweepStakesFactsContentSERP() {
		...
		for (int cycle = 0; cycle < 2; cycle++) {
			...
			CustomLogger.log("Searching with a word to land on SERP page");
			searchResultPage.searchAndGetCount("Hai");

...
public class ReconfigureIWEDevicesTest extends BaseTest {
	...
	public void testCashPrizeForNonSegmentUser() {
		try {
			...
			iweBasePage.driver.maximizeWindow();
			iweBasePage.logIn();
			devicePage.makeGiveawayActive("96");
			// Login to the PCH to get Cash prize
			iweBasePage.driver.resizeWindow(375, 667);
	...
	public void testTokenValueForNonSegmentUser() {
		...
		String desktop_first_search_token_value = joomlaPage.getDesktopFirstSearchTokenValue();
		joomlaPage.unpublishArticle("Prize Machine / Tokens / Consecutive Visit [M]");
		...
		Common.sleepFor(3000);
		devicePage.makeGiveawayInactive("96");
		String non_segement_prize_value_for_2to5 = devicePage.getTokenCatchAllPrizeValue("1506");
		String non_segement_prize_value_for_6to25 = devicePage.getTokenCatchAllPrizeValue("926");

		iweBasePage.driver.resizeWindow(375, 667);
		loginToSearch(randomUser_3);
		for (int i = 1; i < 26; i++) {
			headerPage.search("PCH" + i);
			String activity_msg = headerPage.getmsg().replace(",", "");
			CustomLogger.log("verifying tokens for: " + i + " search");

			// validate tokens for first search
			if (i == 1) {
				Assert.assertTrue(activity_msg.contains(first_search_token_activity_message),
						"Token Activity message doesn't contain the First search message. Actual message is-"
								+ activity_msg + " -and expected is-" + first_search_token_activity_message);
				Assert.assertTrue(activity_msg.contains(desktop_first_search_token_value),
						"Token amount is mismatched. Expected -" + desktop_first_search_token_value
								+ " and the Actual is-" + activity_msg);
			}
			// validate tokens for 2-5 searches
			if (i > 1 && i < 6) {
				Assert.assertTrue(activity_msg.contains(later_first_search_token_activity_message),
						"Token Activity message doesn't contain the later First search message. Actual message is-"
								+ activity_msg + " -and expected is-" + later_first_search_token_activity_message);
				Assert.assertTrue(activity_msg.contains(non_segement_prize_value_for_2to5),
						"Token amount is mismatched. Expected -" + non_segement_prize_value_for_2to5
								+ " and the Actual is-" + activity_msg);
			}
			...
			if (i > 5 && i < 26) {
				Assert.assertTrue(activity_msg.contains(later_first_search_token_activity_message),
						"Token Activity message doesn't contain the later First search message. Actual message is-"
								+ activity_msg + " -and expected is-" + later_first_search_token_activity_message);
				Assert.assertTrue(activity_msg.contains(non_segement_prize_value_for_6to25),
						"Token amount is mismatched. Expected -" + non_segement_prize_value_for_6to25
								+ " and the Actual is-");
			}
		}
	...
	public void testTokenValueForSegmentUser() throws SQLException {
		...
		devicePage.makeGiveawayInactive("1501");
		Common.sleepFor(3000);
		String segement_prize_value_for_2to5 = devicePage.getTokenCatchAllPrizeValue("1511");
		String segement_prize_value_for_6to25 = devicePage.getTokenCatchAllPrizeValue("1468");
		...
		for (int i = 1; i < 26; i++) {
			...
			if (i > 1 && i < 6) {
				Assert.assertTrue(activity_msg.contains(later_first_search_token_activity_message),
						"Token Activity message doesn't contain the later First search message. Actual message is-"
								+ activity_msg + " -and expected is-" + later_first_search_token_activity_message);
				System.out.println(activity_msg);
				Assert.assertTrue(activity_msg.contains(segement_prize_value_for_2to5),
						"Token amount is mismatched. Expected -" + segement_prize_value_for_2to5 + " and the Actual is-"
								+ activity_msg);
			}
			// validate tokens for 6+ searches
			if (i > 5 && i < 26) {
				Assert.assertTrue(activity_msg.contains(later_first_search_token_activity_message),
						"Token Activity message doesn't contain the later First search message. Actual message is-"
								+ activity_msg + " -and expected is-" + later_first_search_token_activity_message);
				Assert.assertTrue(activity_msg.contains(segement_prize_value_for_6to25),
						"Token amount is mismatched. Expected -" + segement_prize_value_for_6to25
								+ " and the Actual is-");
			}
		}
		// Validate the Token Credit calls

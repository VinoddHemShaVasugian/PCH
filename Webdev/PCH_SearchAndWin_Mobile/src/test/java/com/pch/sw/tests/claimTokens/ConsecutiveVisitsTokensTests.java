...
public class ConsecutiveVisitsTokensTests extends BaseTest {
	...
	@Test(enabled = false, description = "configure 'Prize Machine / Tokens / Consecutive Visit [DT]' article in joomla", priority = 3)
	public void testFiveDayMissionTokenConfiguration() {

		// configure article in joomla
		joomlaPage.goToArticle("Prize Machine / Tokens / Consecutive Visit [DT]");
		...
		Assert.assertEquals(joomlaPage.setProperties("Token Amount", propertiesToRead), "success");
	}

	// ,dependsOnMethods="testFiveDayMissionTokenConfiguration"
	@Test(description = "verify user gets awarded for consecutive visits")
	public void ConsecutiveVisit_Day1_To_Day6() throws SQLException, InterruptedException {
		...
		for (int i = 1; i < 7; i++) {
			...
			Assert.assertTrue(homePage.isConsecutiveVisitDisplay(), "Consecutive visit does not display");

			if (i == 5 || i == 6) {
				// for the 5th and 6th consecutive visit
				Assert.assertTrue(homePage.isConsecutiveVisitTokensAwardedForFifthtDay(),
						"Consecutive visit token for day:" + i + " not awarded");
			} else {
				Assert.assertTrue(homePage.isConsecutiveVisitTokensAwardedForCurrentDay(i),
						"Consecutive visit token for day:" + i + " not awarded");
			}

			// Assert.assertEquals(homePage.getMessage(),message,"Meassge:
			...
			if (i < 6) {
				...
				DBUtils.updateFirstSearchCount(randomUser_1.getEmail(), "{\"D\":\"0,\"T\":0,\"M\":0}");

				// update ConsecutiveVisits
				String visitDate = Common.addDaysToDate(currentDate, dateFormat, -1);
				String updatedVisitDate = "a:2:{s:14:\"lastSearchDate\";" + "s:10:\"" + visitDate + "\";"
						+ "s:6:\"visits\";" + "i:" + i + ";}";
				...
				if (Environment.getDevice().equalsIgnoreCase("mobile")) {
					...
					Assert.assertTrue(
							tokenHistoryPage.getActivityName().contains(String.valueOf(propertiesToRead1.get(i - 1))));
		...
		if (!Environment.getDevice().equalsIgnoreCase("mobile")) {
			// Navigate to MyAccount Page page and validate tokens for this
			// visit
			homePage.goToMyAccountPage();
			myAccountPage.navigateToTab(MyAccountTab.Token_History);

			for (int i = 0; i < propertiesToRead1.size(); i++) {
				Assert.assertEquals(tokenHistoryPage.getTokenForEvent(propertiesToRead1.get(i - 1)),
						String.valueOf(propertiesToRead.get(i - 1)));
			}
		}
	...
	public void ConsecutiveVisitTokensNotAwardedForUnpublishedArticle() throws SQLException {
		...
		homePage.closeUserLevelLightBox();
		Assert.assertFalse(homePage.isConsecutiveVisitDisplay(), "Consecutive visit displays");
	...
	public void ConsecutiveVisitTokensNotAwardedForSkipDay() throws SQLException, InterruptedException {
		int day = 2;
		...
		String updatedVisitDate = "a:2:{s:14:\"lastSearchDate\";" + "s:10:\"" + visitDate + "\";" + "s:6:\"visits\";"
				+ "i:1;}";
		DBUtils.updateLastVisitDate(randomUser_1.getEmail(), updatedVisitDate);
		...
		if (Environment.getDevice().equalsIgnoreCase("mobile")) {
			...
			Assert.assertFalse(homePage.isConsecutiveVisitTokensAwardedForCurrentDay(day),

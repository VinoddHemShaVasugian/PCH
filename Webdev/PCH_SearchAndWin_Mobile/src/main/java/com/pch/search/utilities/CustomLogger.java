...
public class CustomLogger {
	...
	public static void createContext() {
		...
		try {
			...
			if (!threadNFileMap.containsKey(threadId)) {
				// System.out.println("thread id "+threadId+" is not found in
				// map..creating new");
				File f = new File(logsDirectory + new SimpleDateFormat("yyyyMMddhhmmss").format(new Date()) + "_"
						+ threadId + ".log");

				/*
				 * File f = new File(System.getProperty("user.dir")+
				 * "\\Reports\\Logs\\"+System.currentTimeMillis()+".log");
				 */

				// System.out.println("Printwriter for file "+threadId+" is
				// created.");
				threadNFileMap.put(threadId, new LogFile(f));

			} else {
				throw new Error(String.format(
						"New Context requested for thread Id - %d while a context is already running.", threadId));
			}
	...
	public static void log(String log) {
		long threadId = Thread.currentThread().getId();
		// Logs written by bots activated by thread should be logged in same
		// File.

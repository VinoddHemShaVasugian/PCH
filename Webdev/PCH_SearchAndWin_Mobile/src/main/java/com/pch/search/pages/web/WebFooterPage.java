...
public class WebFooterPage extends Action {
	...
	public HtmlElement dosAndDontsLink(){
		try{
			return driver.findElement(By.xpath("descendant::a[contains(text(), 'Dos and Don')]"));
		}catch(WebDriverException wde){
			CustomLogger.logException(wde);
			return null;
		}
	...
	public HtmlElement logoBBB(){
		return	driver.findElement(By.xpath("//img[contains(@alt,'Publishers Clearing House LLC BBB ')]"));
	}
	...
	public boolean verifyFooterLinksURL(){
		...
		try{
			...
			verifyLinkURL(aboutPchLink(),"http://info.pch.com/about-publishers-clearing-house");			
			verifyLinkURL(contestDetailsLink(),appUrl+"/aboutsuperprize");			
			verifyLinkURL(customerServiceLink(),"http://pchsearch.custhelp.com/");	
			homeLink().click();
			Common.sleepFor(3000);
			verifyLinkURL(privacyPolicyLink(),"http://privacy.pch.com/searchPrivacyPolicy");			 
			verifyLinkURL(termsOfUseLink(),"http://privacy.pch.com/searchTOU");			

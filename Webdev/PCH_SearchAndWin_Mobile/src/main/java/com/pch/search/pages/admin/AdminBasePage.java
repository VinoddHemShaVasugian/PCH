...
public class AdminBasePage extends Action {
	...
	public HtmlElement getTerms() {
		return driver.findElement(By.xpath(".//*[@id='serp_header_terms']"));
	}

	public HtmlElement mobileTcValue() {
		return driver.findElement(By.id("pch_contest_keys_mobile_tc"));
	}

	public HtmlElement mobileVValue() {
		return driver.findElement(By.xpath(".//*[@id='pch_contest_keys_mobile_v']"));
	}

	public HtmlElement max_searches_before_login() {
		return driver.findElement(By.id("jform_max_searches_before_login"));
	}

	public HtmlElement search_warnings_first() {
		return driver.findElement(By.id("jform_search_warnings_first"));
	}

	public HtmlElement search_warnings_later() {
		return driver.findElement(By.id("jform_search_warnings_later"));
	}

	public HtmlElement aditional_search_warnings() {
		return driver.findElement(By.id("jform_aditional_search_warnings"));
	}

	public HtmlElement Gstestcontent() {
		return driver.findElement(By.id("pch_content_group_4_pch_content"));
	}

	public HtmlElement algo_only_searches_recognized() {
		return driver.findElement(By.id("jform_algo_only_searches_recognized"));
	}

	public HtmlElement algo_only_searches_unrecognized() {
		return driver.findElement(By.id("jform_algo_only_searches_unrecognized"));
	}
	...
	public void OamTool(String uname, String Pwd, String Email) throws IOException {
		String url = "admin." + Environment.getEnvironment() + ".pch.com/onlineaccountmanagement/";
		String BaseUrl = "http://" + uname + ":" + Pwd + "@" + url;
		driver.get(BaseUrl);
		String url1 = "http://admin." + Environment.getEnvironment() + ".pch.com/onlineaccountmanagement/";
		driver.get(url1);
	...
	private String logIn() {
		...
		if (exection_env.equalsIgnoreCase("stg")) {
			driver.get("https://searchadmin.stg.pch.com/administrator/index.php");
		} else if (exection_env.equalsIgnoreCase("qa")) {
			...
			for (HtmlElement element : moduleLink) {
				if (!element.getText().endsWith("(copy)")) {
					if (element.getText().equalsIgnoreCase(module)) {
						try {
							if (driver.findElements(By.cssSelector("span.state.checkedout")).size() > 0) {
								driver.findElement(By.cssSelector("span.state.checkedout")).click();
								Common.sleepFor(2000);
							}
						} catch (Exception e) {
	...
	public void setExternalURLValue(String value) {
		HtmlElement ele = driver.findElement(By.id("pch_content_group_1_pch_content_external_url"));
		ele.clear();
		ele.sendKeys(value);
	...
	public void testValidSSO(String links) {
		...
		if (presentURL.contains("guidedsearch")) {
			presentURL = presentURL.replace("guidedsearch", "");
		}
		...
		for (int i = 0; i < myList.size(); i++) {
			...
			try {
				if (!myList.get(i).equals("logout")) {
					System.out.println(presentURL + "" + myList.get(i));
					driver.get(presentURL + "" + myList.get(i));
					// System.out.println(presentURL+""+myList.get(i));
	...
	public List<String> serpRecognizedMsgsForMobile(String article) {
		goToArticle(article);
		...
		String mobileFirstID = "serp_messaging_recognized_mobile_first_search";
		String mobileLaterID = "serp_messaging_recognized_mobile_later_search";

		HtmlElement mobileFirst = driver.findElement(By.id(mobileFirstID));
		HtmlElement mobileLater = driver.findElement(By.id(mobileLaterID));

		String mobileFirstText = mobileFirst.getAttribute("value");
		String mobileLaterText = mobileLater.getAttribute("value");

		List<String> mobileRecogText = new ArrayList<String>();
		mobileRecogText.add(mobileFirstText);
		mobileRecogText.add(mobileLaterText);

		// System.out.println(desktopUnRecogText);

		return mobileRecogText;
	}

	public List<String> serpUnRecognizedMsgsForMobile(String article) {
		goToArticle(article);
		...
		String mobileFirstID = "serp_messaging_unrecognized_mobile_first_search";
		String mobileLaterID = "serp_messaging_unrecognized_mobile_later_search";
		...
		List<String> mobileUnRecogText = new ArrayList<String>();
		mobileUnRecogText.add(mobileFirstText);
		mobileUnRecogText.add(mobileLaterText);

		// System.out.println(desktopUnRecogText);

		return mobileUnRecogText;
	}

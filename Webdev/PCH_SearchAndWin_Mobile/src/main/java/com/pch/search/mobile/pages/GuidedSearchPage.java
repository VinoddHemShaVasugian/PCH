...
//		return driver.findElement(By.id("searchField0"));
	...
	public HtmlElement home(){
		return driver.findElement(By.xpath("//ul//a[contains(text(), 'Home')]"));
	}
	
	public HtmlElement myAccont(){
		return driver.findElement(By.xpath(".//*[@id='site-nav']/ul/li/a[contains(text(),'My Account')]"));
	}
	...
	*/
	...
	public void clickBBBandVerifyTitle(){
		...
		//Checking whether we are seeing a new window are not
		if(driver.getWindowHandles().size()>1){
			CustomLogger.log("Validating the URL of BBB.");
			driver.switchToChildWindow(parentWindow);					
			BBBTitle = driver.getTitle();
			CustomLogger.log("Verifying the URL of BBB on GS Search and Win page - "+ BBBTitle);
			Assert.assertTrue(BBBTitle.contains("NY - BBB Business Review - BBB serving Metropolitan New York, "));
			driver.close();
			...
			BBBTitle = driver.getTitle();
			CustomLogger.log("Verifying the URL of BBB on GS Search and Win page - "+ BBBTitle);
			//Assert.assertEquals(driver.getCurrentUrl(),"http://www.bbb.org/new-york-city/business-reviews/marketing-programs-and-services/publishers-clearing-house-llc-in-port-washington-ny-22145/#bbbonlineclick");
			Assert.assertTrue(BBBTitle.contains("NY - BBB Business Review - BBB serving Metropolitan New York, "));
			CustomLogger.log("Verified BBB logo and URL on GS_Page.");
		}
	}
	...
	public void clickeTrusteandVerifyTitle(){
		...
		if(driver.getWindowHandles().size()>1){
			...
			driver.switchToChildWindow(parentWindow);					
			eTrusteTitle = driver.getTitle();
			CustomLogger.log("Verifying the URL of eTruste on GS Search and Win page - "+ eTrusteTitle);
			Assert.assertTrue(eTrusteTitle.contains("online privacy and online safety are certified by TRUSTe"));
			driver.close();
			driver.switchTo().window(parentWindow);

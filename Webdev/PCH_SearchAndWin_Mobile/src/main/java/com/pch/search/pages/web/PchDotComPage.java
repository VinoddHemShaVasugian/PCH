...
public class PchDotComPage extends Action {
	...
	public void load() {
		CustomLogger.log("Loading :" + "http://www." + Environment.getEnvironment() + ".pch.com");
	...
	public SelectList dobMonthListBox() {
		return driver.findSelectList(By.xpath("//select[contains(@class,'DOBMonth')]"));
	}

	public SelectList dobDayListBox() {
		return driver.findSelectList(By.xpath("//select[contains(@class,'DOBDate')]"));
	}

	public SelectList dobYearListBox() {
		return driver.findSelectList(By.xpath("//select[contains(@class,'DOBYear')]"));
	}
	...
	public void EnterUserDetails(User user) {
		...
		try {
			if (driver.findElement(By.id("ST")).isDisplayed()) {
				setState(user.getState());
			}
		} catch (Exception e) {
			System.out.println("No State Field on the page");
			e.printStackTrace();
		}
		
		try {
			if (driver.findElement(By.id("CI")).isDisplayed()) {
				setCity(user.getCity());
			}
		} catch (Exception e) {
			System.out.println("No City Field on the page");
			e.printStackTrace();
		}

		try {
			if (driver.findElement(By.cssSelector("div.dob > h3")).isDisplayed()) {
			...
			System.out.println("No DOB Field on the page");
			e.printStackTrace();
		}
	}
	...
	public void skipAds() {
		...
		// HtmlElement continueToNextPage =
		// driver.findElement(By.id("mpNavNext"));
	...
	/*
	 * #To remove Tutorial Holder# When we complete registration in PCH.com site
	 * we will navigate to adds page and when we skip those ads it'll take us to
	 * PCH home page with Tutorial Holder on top of it.
	 */
	 ...
	 * 
	 * driver.findElement(By.xpath(".//*[@id='main']/div[2]/div/div/div[1]/h3"))
	 * .scrollDownAndClick(); String gameCountString =
	 * driver.findElement(By.id("gameCount")).getText();
	 * 
	 * int gameCount = Integer.parseInt(gameCountString);
	 * 
	 * for(int i=1; i<=gameCount; i++){
	 * 
	 * screen.click("GamePathImages/playNow.PNG"); Common.sleepFor(5000);
	 * screen.wait("GamePathImages/revealAll"+i+1+".PNG", 5000);
	 * screen.click("GamePathImages/revealAll"+i+".PNG");
	 * Common.sleepFor(20000); screen.wait("GamePathImages/continue.PNG",
	 * 10000); screen.click("GamePathImages/continue.PNG");
	 ...
	 * screen.click("GamePathImages/playNow.PNG"); Common.sleepFor(5000);
	 * screen.wait("GamePathImages/revealAll"+i+1+".PNG", 5000);
	 * screen.click("GamePathImages/revealAll2.PNG"); Common.sleepFor(20000);
	 * screen.wait("GamePathImages/continue.PNG", 10000);
	 * screen.click("GamePathImages/continue.PNG"); Common.sleepFor(50000);
	 * 
	 * screen.click("GamePathImages/playNow.PNG"); Common.sleepFor(5000);
	 * screen.wait("GamePathImages/revealAll"+i+1+".PNG", 5000);
	 * screen.click("GamePathImages/revealAll3.PNG"); Common.sleepFor(20000);
	 * screen.wait("GamePathImages/continue.PNG", 10000);
	 * screen.click("GamePathImages/continue.PNG"); Common.sleepFor(50000);
	 * 
	 * screen.click("GamePathImages/playNow.PNG"); Common.sleepFor(5000);
	 * screen.wait("GamePathImages/revealAll"+i+1+".PNG", 5000);
	 * screen.click("GamePathImages/revealAll4.PNG"); Common.sleepFor(20000);
	 * screen.wait("GamePathImages/continue.PNG", 10000);
	 * screen.click("GamePathImages/continue.PNG"); Common.sleepFor(50000);
	 * 
	 * screen.click("GamePathImages/playNow.PNG"); Common.sleepFor(5000);
	 * screen.wait("GamePathImages/revealAll"+i+1+".PNG", 5000);
	 * screen.click("GamePathImages/revealAll5.PNG"); Common.sleepFor(20000);
	 * screen.wait("GamePathImages/continue.PNG", 10000);
	 * screen.click("GamePathImages/continue.PNG"); Common.sleepFor(50000);
	 * 
	 * }

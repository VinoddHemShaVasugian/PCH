...
public abstract class Action {
	...
	public void switchToLandscapeMode(int width, int hight) {
		driver.manage().window().setSize(new Dimension(width, hight));
	}

	/**
	 * This function is to validate the URL when new tab is opened upon click on
	 * any element
	 * 
	 * @param: Link
	 *             for which validating the url
	 ...
	 * @param by
	 */
	public boolean waitForElementInVisible(By by, int noOfTimesToRefresh) {
		int count = 0;
	 ...
	 * @param by
	 */
	public boolean waitForElementVisible(By by, int noOfTimesToRefresh) {
		int count = 0;
		while (count < noOfTimesToRefresh) {
			try {
	 ...
	 * @return: HtmlElement
	 */
	public HtmlElement waitForElementToBeVisible(WebElement ele) {
		HtmlElement element = null;
		int attempts = 0;
		while (attempts < 2) {
			try {
				element = driver.waitForElementToBeVisible(ele, timeUnit);
			} catch (ElementNotVisibleException e) {
				CustomLogger.log("Got: " + e.getClass().getSimpleName() + " for " + ele);
				CustomLogger.log("Trying againg to find the element");
			}
			attempts++;
		}

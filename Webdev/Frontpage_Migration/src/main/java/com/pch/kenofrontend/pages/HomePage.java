...
public class HomePage extends PageObject {
	...
	private By Register_btn = new By.ByCssSelector("a.uninav__registration.register-btn");
	private By SignIn_btn = new By.ByLinkText("Sign In");
	//private By QuickPick_btn = new By.ByClassName("card__controls__btn--quick");
	private By QuickPick_btn = new By.ByXPath("//button[contains(text(),'Quick Pick')]");
	...
	//private By CardTabNum = new By.ByXPath("//span[contains(@class,'card-tab__num')][contains(text(),'02')]");
	//private By CardTabStatus = new By.ByXPath("//div[contains(@class,'card-tab--unlocking-progress card-tab--unlocking-out card-tab--unlocked')]//div//p[contains(@class,'card-tab__status')][contains(text(),'UNLOCKED')]");
	private By CardTabStatus = new By.ByCssSelector("div.card-tab--unlocking-progress > div > div.card-tab__unlocked > p.card-tab__status");
	private By CardTabNum = new By.ByCssSelector("div.card-tab--unlocking-progress > div > span");
	
	private By GameFrame = new By.ByXPath("//div[@id='htmlgame_frame_div']");
	public By Drawn_Clock = By.cssSelector("div.kenobar.kenobar--live > div.kenobar__body.container > div.kenobar__clock > div.clock.clock--countdown > div.clock__time > span[class='clock__time__header'][style='visibility: hidden;']");
	public By liveDrawing = By.linkText("Live Drawing");
	private By DrawnNumbers = new By.ByCssSelector("section#drawing__number-pad > div.drawing__ball.drawing__ball--drop > span");
	...
	private By tutorialsBtn = new By.ByXPath("//button[contains(text(),'How It Works')]");
	private By tutorialsNextBtn = new By.ByXPath("//button[contains(text(),'Next')]");
	private By tutorialsPreviousBtn = new By.ByXPath("//button[contains(text(),'Previous')]");
	//private By tutorialsCloseBtn = new By.ByCssSelector("div.modal.fade.tutorial.in > div > div > div.modal-header > button");
	private By tutorialsCloseBtn = new By.ByCssSelector("div.modal.fade.tutorial.in > div > div > div.modal-header > button");
	private By tutorialScreenHeader = new By.ByCssSelector(".tutorial__screen__header");
	...
	//private By OPTIN_light_box = new By.ByClassName("close");
	//private By OPTIN_light_box = new By.ByXPath("//div[@class='modal-content pre-opt']//span[@aria-hidden='true'][contains(text(),'×')]");
	...
	private By Optin_signup_button = new By.ByXPath("//button[contains(text(),'Sign Up Now!')]");
	private By Optin_thankYou_message = new By.ByXPath("//h4[contains(text(),\"We're So Glad You Signed Up!\")]");
	private By univnav = new By.ByCssSelector(".uninav__carousel");
	private By univnav_properties = new By.ByXPath("//*[@class='uninav__carousel__navlist']/li/a");
	...
	private By selectedNumber = new By.ByCssSelector("form.card__form > ol.card__numbers > li.card__number--selected > label.card__number__label > input.card__number__input");
	private By dailyTokenBonus_button = new By.ByCssSelector("button.btn--gold");
	private By dailyTokenBonusModuleTokens= new By.ByCssSelector("div.modal-body > ol.dailybonus__values > li.dailybonus__value--complete > span.dailybonus__value__bonus");
	private By dailyTokenBonusModuleVisitNumber= new By.ByCssSelector("div.modal-body > ol.dailybonus__values > li.dailybonus__value--complete > p.dailybonus__value__visits > span.dailybonus__value__visits__body"); 
	...
	private By leaderboard_mobile_header = new By.ByCssSelector(
			".leaderboard--scoreboard.dark>h3");
	private By leaderboard_yesterday_winners = new By.ByCssSelector(
			".leaderboard--yesterday-view");
	private By leaderboard_yesterday_winners_details = new By.ByCssSelector(".winner");
	...
	public void clickOn_SignIn_Btn() { //Vinoth - No change
	...
	public void playScratchCards() throws InterruptedException {
		try {
			...
			if (element(Playnowbtn_frame).isDisplayed()) {
				System.out.println("Play Now button is still appearing so clicked again");
				element(Playnowbtn_frame).click();
			}
			...
			System.out.println("Keep going button visible"+isElementVisible(Keepgoing_btn)); 
			scrollPage();
	...
	public void HandleVideoAdvertisement() throws InterruptedException{
		...
		try{
			if ((element(AdvContainer).isPresent())||(element(AdvContain).isPresent())) {
			System.out.println("Video Advertisement Frame is Available, going to force skip   ");
			JavascriptExecutor js = (JavascriptExecutor) getDriver();
			js.executeScript("jwplayer().trigger('adComplete')");
		    }
			else{
				System.out.println("Due to error while skipping video ad, we are forcing wait for 15 seconds.");
				WaitHelper.forceWait(15000);
			}
				
		}
		catch(Exception e){
			System.out.println("Faced exception while skipping video ad. Hence forcing wait of 15 seconds.");
			WaitHelper.forceWait(15000);
		}
	...
	public void clickOn_Submit_Btn_AfterQuickPick(int index) {
		...
		List<WebElement> selecetdNumbers = getDriver().findElements(By.cssSelector("#main-content > article[data-cardnum=\""
								+ index
...
//		Assert.assertTrue("User Login / Registration got Passed",
//				element(SignOut_link).isVisible());
		Assert.assertTrue(element(SignOut_link).isVisible());
	}
	...
	public void skipAdd() {
		...
		try {
			JavascriptExecutor js = (JavascriptExecutor) getDriver();
			 js.executeScript("jwplayer().trigger('adComplete')");
			System.out.println("Entered Video Advertisement Frame to Skip If SKip Button Available");
			WebElement AdvFrame = getDriver().findElement(By.xpath("//iframe[contains(@src,'https://securepubads.g.doubleclick.net')]"));
	...
	public void moveToPathGameFrame() {
		try {
			WebElement PathGameFrame = getDriver()
					.findElement(By.xpath("//background-image:url[contains(@src,'//pathgamesassets')]"));
	...
	// Changes for Coaching Screen story
		public void coachingTickingClock() {
			waitForAnElement(coaching_screen);
			if (element(coaching_screen).isDisplayed() && element(coaching_screen).getText().contains("The Clock is Ticking! Get all 5 Keno Cards in for the Drawing")) {
				System.out
...
//		WaitHelper.forceWait(10000);
		//waitForAnElement(Playnow_btn);
		//System.out.println("this is Play now of choices screen        "+ Playnow_btn);
	...
	public void scratchpathplay() throws Exception {
		...
		try {
			waitForAnElement(Playnow_btn);
			System.out.println("identifiable button playnow"+element(Playnow_btn).isCurrentlyEnabled());
			scrollPage();			
			element(Playnow_btn).click();
			waitForAnElement(GameFrame);
			...
			for (scratchGames = 1; scratchGames <= totalNumberOfGames; scratchGames++) {
				waitForAnElement(Playnowbtn_frame);
				System.out.println("Is PLAY NOW visible/clickable ?"+isElementVisible(Playnowbtn_frame));
				element(Playnowbtn_frame).click();
				WaitHelper.forceWait(5000);
				getDriver().switchTo().parentFrame();
...
//		((JavascriptExecutor) getDriver()).executeScript("scroll(0,200)");
		((JavascriptExecutor) getDriver()).executeScript("arguments[0].scrollIntoView(true);", element(Submit_btn));
...
//		a1.moveToElement(e1).click().perform();
	...
	public void playSecondKenoCard()
	{
		...
		if ((element(CardTabStatus).getCssValue("visibility").equalsIgnoreCase("visible")) && (element(CardTabNum).getText().equalsIgnoreCase("02")))
		{
...
//		rs.generateGoldUserInSTG();
               ...
               try {
                     if (getDriver().getTitle().contains("keno")) {
                            ...
                            if(getDriver().getCurrentUrl().contains("blackjack")){
                                   Assert.assertTrue( propertynames.get(i).toString().contains("pch.com/bl"));
                            }
                            else{
                     ...
                     System.out.println("Element to click is not visible. Therefore moving right");
	...
	public void scrollnavigation() {
        ...
        while (!element(uninav_next).getAttribute("class").contains("disabled")) {
               System.out.println("Scrolling right of the universal navigation and number of times it scrolled is: "
                                          + count);
               element(uninav_next).click();
               count++;
        }
        ...
        while (!element(uninav_previous).getAttribute("class").contains(
                     "disabled")) {
               System.out.println("Scrolling left of the universal navigation and number of times it scrolled is: "
                                          + count);
	...
	public void verifyOptinLightboxPresent(){
		WaitHelper.waitForPresenceOfElement(getDriver(),Optin_signup_button);
	...
	public void clickOnOptinSignupButton()
		 {
			 WaitHelper.waitForElementToBeClickable(getDriver(),Optin_signup_button,2);
			 element(Optin_signup_button).click();
		 }

	public void verifyOptinThankYouMessage() 
		{
			WaitHelper.waitForPresenceOfElement(getDriver(), Optin_thankYou_message);
	...
	public void verifyOptinTokens() throws InterruptedException
		...
		Iterator <WebElement> i2 = creditTokens.iterator();
			...
			while(i.hasNext() && (i2.hasNext()))
			{
				...
				Assert.assertTrue(e1.getText().contains("Email Sign Up Bonus Tokens!"));
				}
			}
	...
	public void verifyTutorialsOpen()
	{
		...
		for (int i=0; i<6; i++)
		{
			...
			{
				openedScreenHeading = element(tutorialScreenHeaders.get(i)).getText();
				currentScreenNumber = i+1;
				break;
			}
		}
		
		System.out.println("Heading of open tutorial screen:: " + openedScreenHeading);		
		System.out.println("Tutorial Screen " + currentScreenNumber + " is open");	
		
		expectedScreenHeader.add("Play PCHkeno and You Could Win $1,000,000.00!");
	...
	{		
		for (int i=0; i<(6-currentScreenNumber); i++)
		{
			...
			element(tutorialScreenImages.get(currentScreenNumber+i)).waitUntilVisible();
			element(tutorialScreenHeaders.get(currentScreenNumber+i)).waitUntilVisible();			
			if (element(tutorialScreenHeaders.get(currentScreenNumber+i)).isCurrentlyVisible())
			{
				openedScreenHeading = element(tutorialScreenHeaders.get(currentScreenNumber+i)).getText();
				System.out.println("openedScreenHeading:: " + openedScreenHeading);
				System.out.println("Tutorial Screen " + (currentScreenNumber+i+1) + " is open");
			}else
			{
				System.out.println("Screen " + (currentScreenNumber+i+1) + " is not open");
				Assert.assertFalse(true);
			}			
			
			//Checking if the fetched header text is expected or not
			if (openedScreenHeading.equals(expectedScreenHeader.get(currentScreenNumber+i)))
			{
	...
	public void clickTutorialsPrevious()
	{
		for (int i=0; i<(5); i++)
		{
			...
			{
				openedScreenHeading = element(tutorialScreenHeaders.get(4-i)).getText();
				System.out.println("openedScreenHeading:: " + openedScreenHeading);
	...
	public void clickTutorialsClose()
	{
		...
		try 
		{
			...
			System.out.println("SYSO:: Tutorial screen closed. Home page appearing.");
		} catch (org.openqa.selenium.NoSuchElementException e) 
		{
	...
	{
		//This method can be used to verify when below users category completes registration and returns to Home Page -
		//- Mini Reg User
		//- Social User
		...
		if (!element(completeRegBtn).isCurrentlyVisible())
			System.out.println("Complete Registration button not appearing - As Expected.");
		else
			Assert.assertFalse("Complete Registration button still appearing", true);		
	}
    ...
    {   
       List<WebElement> numbersSubmitted = getDriver().findElements(selectedNumber);
       System.out.println("numbersSubmitted.size()  "+ numbersSubmitted.size());
       
       numbersSubmittedAre.clear();
       for (int i=0; i<10; i++)
       {
       	numbersSubmittedAre.add(numbersSubmitted.get(i).getAttribute("value"));
       }
	...
	{
		List<WebElement> sideRailNumbers = getDriver().findElements(By.cssSelector("div.card-tab-holder > div.card-tab--completed > div > div.card-tab__completed > ul.card-tab__completed__nums > li.card-tab__completed__num > span"));
		...
		System.out.println("numbers on side Rail " +Arrays.toString(sideRailNumbersarr));
	...
	public void verifyDailyTokenBonusTokens() 
	{
			if (element(dailyTokenBonus_button).isPresent())
			{	
				System.out.println("Daily Token Bonus Module Tokens   " + element(dailyTokenBonusModuleTokens).getTextValue());
				System.out.println( " Module visit number " +element(dailyTokenBonusModuleVisitNumber).getText());
				int visit = Integer.parseInt(element(dailyTokenBonusModuleVisitNumber).getTextValue());
				System.out.println("visit  = "+visit);
				System.out.println("JoomlaAdminPage.dailyTokenBonusArr nth visit "+JoomlaAdminPage.dailyTokenBonusArr[visit-1]);
				Assert.assertEquals(JoomlaAdminPage.dailyTokenBonusArr[visit-1], element(dailyTokenBonusModuleTokens).getTextValue());
			}
			else 
			{
...
public void observeLeaderboard() {
	...
	if (element(leaderboard_mobile_icon).isCurrentlyEnabled()) {
		...
		System.out.println("***header is: "
				+ element(leaderboard_mobile_header).getText());
		Assert.assertTrue(element(leaderboard_mobile_header).getText()
				.contains("MOBILE"));
	} else
...
public void winnersAppearingInLeaderboard() throws Exception {
	...
	if (element(leaderboard_yesterday_winners).isCurrentlyEnabled()) {
		...
		System.out.println("***winner details are: "
				+ element(leaderboard_yesterday_winners_details).getText());
	 else

...
Feature: Verify the badges awarding feature for Influencer Pro Plus badge.
  ...
  @Desktop @Mobile
  Scenario Outline: User complete 3 surveys and verify the influencer pro plus badge awarded for "<userType>" user.
  ...
  @Desktop @Mobile
  Scenario Outline: User completes 3 surveys and verify the influencer pro plus badge awarded for "<userType>" user.
    Given I land on the Surveys Page as a "<userType>" user
    When I close the Take a Tour popup
    #And I click first Take Survey Button
    And I navigates to survey 'Auto_Lucid_Tokens'

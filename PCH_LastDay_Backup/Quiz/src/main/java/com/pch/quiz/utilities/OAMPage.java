...
public class OAMPage extends PageObject {
	...
	public void enterUserWithoutPwdEmail() throws InterruptedException
	{
		...
		System.out.println(" this is  email " + RegistrationPage.userEmail);
		WaitHelper.waitForElementToBeClickable(getDriver(), EmailTextBox,30);
		WaitHelper.forceWait(7000);
		element(EmailTextBox).typeAndEnter(RegistrationPage.userEmail);
	}
	...
	public void enterUserWithPwdEmail(String email) throws InterruptedException
	{
		...
		WaitHelper.waitForElementToBeClickable(getDriver(), EmailTextBox,30);
		WaitHelper.forceWait(7000);
		element(EmailTextBox).typeAndEnter(email);
	}
	...
	public void verifyContestEntryInOam(String expectedContestKey) throws InterruptedException {
		WebElement contestKeyInOAM = getDriver().findElement(By.xpath("//div[@class='x-grid-cell-inner'][contains(text(),'B')]"));
		String actualContestKey=contestKeyInOAM.getText();
		System.out.println("actualContestKey-->"+actualContestKey);
		WaitHelper.forceWait(1000);
		Assert.assertEquals(expectedContestKey, actualContestKey);
	}

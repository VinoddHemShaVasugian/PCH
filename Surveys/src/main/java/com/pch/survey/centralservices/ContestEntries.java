...
public class ContestEntries {
	...
	public List<UserEntries> verifyContestEntry() {
		List<UserEntries> userEntry;
		if (envir.equalsIgnoreCase("QA")) {
			userEntry = ContestEntriesHelper.getContestEntriesByEmailQA(Registrations.getEmail());
		} else {
			userEntry = ContestEntriesHelper.getContestEntriesByEmailSTG(Registrations.getEmail());
		}
		return userEntry;
	}

	public static void main(String[] args) {
		List<UserEntries> entriesByEmail = (new ContestEntriesHelper())
				.getContestEntriesByEmailCQA("csacb16c75b-6993-4.03.09.23@pchmail.com");
